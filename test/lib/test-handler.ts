...
/** Lib */
import { DataSetup } from './exporter';
...
class TestHandler {
     ...
     */
    constructor( @inject('DataSetup') private dataSetup: DataSetup, name: string, testCaseFunction: (assert: test.Test) => Promise<void>) {
        this.testName = name;
        ...
        return new Promise((resolve, reject) => {
            test(this.testName, async (t: test.Test) => {
                try {
                    ...
                    await this.dataSetup.clearTables();
